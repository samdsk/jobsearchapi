...
class Collector {
   ...
   * @returns an Object containing a summery of collected data and an array of actual data obtained from API
   */
  async searchJobsByType(JobType, RequestOptions) {
    ...
    while (data.jobCount && data.jobCount >= 10 && count < 5) {
    ...
    console.log(
      `Collected: ${searchResults.jobs.length} from JobType: ${JobType} Location:${searchResults.location} Language:${searchResults.language}`
    );
