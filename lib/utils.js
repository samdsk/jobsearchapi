...
const { DupJob } = require("../schemas/DupJob");
...
async function transferDataBase(url) {
  ...
  const total = await DupJob.countDocuments();
  let count = 0;
  console.log(total);
  for await (const job of DupJob.find()) {
    const jobPost = JobPostConverter.convert(job, job.job_type);
    try {
