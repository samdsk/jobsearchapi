...
describe("Label Schema", () => {
  ...
  test("Delete Domain and cascade delete to Annotations and Tokens", async () => {
    await expect(Domain.deleteOne({ _id: domain._id })).resolves.not.toThrow();
  ...
  test("Cant delete Label without an id", async () => {
    await expect(Domain.deleteOne({ job_type: "jobtype" })).rejects.toThrow(
      "usage: Domain.deleteOne({_id:id})"
    );
  });
});
