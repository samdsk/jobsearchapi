...
describe("Label Schema", () => {
  ...
  test("Delete Label and cascade delete to Annotations and Tokens", async () => {
    await expect(Label.deleteOne({ _id: label._id })).resolves.not.toThrow();
  ...
  test("Cant delete Label without an id", async () => {
    await expect(Label.deleteOne({ job_type: "jobtype" })).rejects.toThrow(
      "usage: Label.deleteOne({_id:id})"
    );
  });
});
