...
describe("API call testing", () => {
  it("should send an API request and receive list of jobs", async () => {
    ...
    jest.spyOn(mockSender, "sendJobSearchRequest").mockImplementation(() => {
      return response_example_2;
    });
    const collector = new Collector(mockSender);

    const response = await collector.searchJobsByType(jobType);
    expect(response.collected).toBe(response_example_2.jobs.length);
    expect(mockSender.sendJobSearchRequest).toHaveBeenCalledTimes(1);
  });

  it("should send 2 API requests and receive list of jobs", async () => {
    const jobType = "Software Engineer";
    const mockSender = new SearchRequestSender();

    jest
      .spyOn(mockSender, "sendJobSearchRequest")
      .mockImplementationOnce(() => response_example)
      .mockImplementation(() => response_example_2);

    const collector = new Collector(mockSender);

    const response = await collector.searchJobsByType(jobType);
    expect(response.collected).toBe(
      response_example_2.jobs.length + response_example.jobs.length
    );
    expect(mockSender.sendJobSearchRequest).toHaveBeenCalledTimes(2);
  });

  it("should call searchJobsByType twice", async () => {
    const jobTypes = ["Software Engineer", "Cuoco"];

    const collector = new Collector();
    jest.spyOn(collector, "searchJobsByType").mockImplementation(() => {});

    const response = await collector.searchJobTypeList(jobTypes);
    expect(collector.searchJobsByType).toHaveBeenCalledTimes(2);
    expect(response[0].jobType).toBe(jobTypes[0]);
    expect(response[1].jobType).toBe(jobTypes[1]);
  });
});
...
describe("insert a job details to db", () => {
  it("should call insertAllJobs 1 time", async () => {
    ...
    jest
      .spyOn(mockSender, "sendJobSearchRequest")
      .mockImplementation(() => response_example_3);

    const collector = new Collector(mockSender);
    const response = await collector.searchJobsByType("Cuoco");

    expect(mockSender.sendJobSearchRequest).toHaveBeenCalledTimes(1);
    expect(InsertJob.insertAllJobs).toHaveBeenCalledTimes(1);
  });

  it("db should contain 2 elements of given id", async () => {
    const mockSender = new SearchRequestSender();
