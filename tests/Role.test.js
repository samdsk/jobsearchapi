...
describe("Label Schema", () => {
  ...
  test("Delete Role and cascade delete to Annotator and Annotations", async () => {
    await expect(Role.deleteOne({ _id: role._id })).resolves.not.toThrow();

    expect(await Role.findById(role._id)).toBe(null);
  ...
  test("Cant delete Label without an id", async () => {
    await expect(Role.deleteOne({ role: "Role" })).rejects.toThrow(
      "usage: Role.deleteOne({_id:id})"
    );
  });
});
